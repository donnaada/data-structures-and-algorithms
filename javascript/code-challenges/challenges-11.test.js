...
function transformToLis(obj){
  // Solution code here...
  let newArr = Object.entries(obj).map(([key, value]) => `<li>${key}: ${value}</li>`);
  return newArr;
}
...
const count = (target, input) => {
  // Solution code here...
  let newArr = input.reduce((count,arr)=> count + arr.filter(num => num === target).length, 0);
  return newArr;
};
...
const totalSum = (input) => {
  // Solution code here...
  let newArr = input.reduce((acc, curNum)=> acc + curNum.reduce((sum, nums) => sum + nums, 0),0);
  return newArr;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filterNum = input.map(numArr => numArr.filter(num => (typeof num) === 'number' && num % 5===0));
  return filterNum.map(numarray => numarray.map(num => Math.pow(2, num)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let isMaleFemale = data.filter(character => character.gender === 'female' || character.gender === 'male');
  return isMaleFemale.map(character => character.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let findShortestCharacter =  data.filter(character => character.height).reduce((acc, curChar) => {
    return parseInt(curChar.height) < parseInt(acc.height) ? curChar : acc;
  });

  return findShortestCharacter.name;
};
