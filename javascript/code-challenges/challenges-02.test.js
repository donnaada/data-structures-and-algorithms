...
const raisedToTheThird = (arr) => {
  // Solution code here...
  // let thirds = arr.map(function(num){
  //   let power = 3;
  //   return Math.pow(num,power);
  // });
  let thirds = arr.map(num=>Math.pow(num,3));
...
const charCode = (arr) => {
  ...
  arr.forEach(index => {
    charArr.push(index.charCodeAt(0));
  });
...
const evenOdd = (arr) => {
  ...
  arr.forEach(num =>{
    isNaN(num) ? evenOdd.push('N/A') : num % 2=== 0 ? evenOdd.push('even') : evenOdd.push('odd');
  });
...
const extractAbilities = (arr) => {
  ...
  let abilityName = [];
  arr.forEach(ability => {
    abilityName.push(ability.ability.name);
  });
...
const extractStats = (arr) => {
  ...
  let pokeState =[];
  arr.forEach(stats =>{
    let total = stats.effort + stats.baseStat;
    let name = stats.stat.name;
    pokeState.push({name, total});
  });
