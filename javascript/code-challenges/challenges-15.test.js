...
const screenForNames = (arr) => {
  // Solution code here...
  let regexp = /^((Mr|Ms|Mrs|Dr)\. [a-zA-Z]+)$/gm;
  return arr.filter(name => regexp.test(name));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let charactersBigger = arr.filter(data => data.mass > 77);

  let resultString ='';

  charactersBigger.forEach(character =>{
    resultString === '' ? resultString += character.name : resultString += ' - ' + character.name;
  });
  return resultString;
};
...
const detectTicTacToeWin = (board) => {
  ...
  const helpCheck = (row1, col1, row2, col2, row3, col3) => {
    let firstRow = board[row1][col1];
    let secondRow = board[row2][col2];
    let thirdRow = board[row3][col3];
    if (!firstRow) {
      return false;
    }
