...
const updateNumbers = (obj) => {
  // Solution code here...
  let updatedList = Object.entries(obj).map(([name, number]) => `${name}: ${number}`)
  return updatedList;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const searchCharacter = arr.find(char => char.name === character);
  if (!searchCharacter || !searchCharacter.children || Object.keys(searchCharacter.children).length === 0) {
    return false;
  }
  return true;
};
